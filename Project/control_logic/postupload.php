        ...
        else if(isset($_COOKIE["restaurantID"]))
        {
            ...
            if(is_uploaded_file($_FILES["photoInput"]["tmp_name"]))
            {
                ...
                if($photoCheck != null)
                {
                    ...
                    if(!($photoType == "png" || $photoType == "jpg" || $photoType == "jpeg"))
                    {
                        header("Location:../restaurantposts.php?errormsg=Allowed Image Formats - png, jpg, jpeg !&postText=$postText");
                        exit;
                    }
                    ...
                    {
                        createDatabaseConnection();
                        $query="insert into photos (account_id, storage_location, width, height) values ($resID, '$retrieveDirectory', $width, $height)";
                        executeQuery($query);
                        $photoID=getLastID();
                        closeDatabaseConnection();
                    }
                    ...
                    {
                        header("Location:../restaurantposts.php?errormsg=Error Occurred ! Image Could Not Be Uploaded !&postText=$postText");
                        exit;
                    }
                }
                else
                {
                    header("Location:../restaurantposts.php?errormsg=Please Upload An Image File !&postText=$postText");
                    exit;
                }
            ...
            if($photoID != -1)
            {
                $finalQuery="insert into posts (account_id, post_text, photo_id) values ($resID, '$postText', $photoID)";
            }
            else if($photoID == -1)
            {
                $finalQuery="insert into posts (account_id, post_text, photo_id) values ($resID, '$postText', null)";
            }
            ...
            header("Location:../restaurantposts.php?successMsg=your post has been created !");
        }
    }
?>
