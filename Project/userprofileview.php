<?php
    ...
    $id=$accountID=$username=$profileUsername=$directory=$width=$height="";
    $firstname=$lastname=$gender=$dateOfBirth=$email=$location="";
    $followers=0;

    if(isset($_COOKIE["userID"]) || isset($_COOKIE["restaurantID"]))
    {
        $id=$_GET["profileID"];
        ...
        if(count($result11) == 0)
        {
            // AVOIDING PROFILE VIEW ACCESS FOR INVALID GENERAL USERS ID
